...
if prog.exists():
    ...
    stack = d.get("stack") or "product"
elif (root / ".cursor/stack-selection.json").exists():
    stack = json.loads((root / ".cursor/stack-selection.json").read_text(encoding="utf-8")).get("stack", "product")
if not feature:
    print("")
    raise SystemExit(0)
...
if stack == "product":
    if feature:
        paths += [
            f"src/SpatialLabsOptimizer/{feature}",
            f"site/catalog/src/{feature}",
            f"workers/steam-library/src/{feature}",
        ]
    else:
        paths += ["src/SpatialLabsOptimizer", "site/catalog/src", "workers/steam-library/src"]
