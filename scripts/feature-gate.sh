...
# Usage: scripts/feature-gate.sh [--json] [--stack web|python|android|node|product|multi] [--step LABEL]
set -euo pipefail
...
if [ "$STACK" = "product" ]; then
  if ! command -v dotnet >/dev/null 2>&1; then
    block_env "dotnet not found; required for WinUI product gate"
  fi
  if [ -f SpatialLabsOptimizer.sln ]; then
    run_cmd dotnet-build dotnet build SpatialLabsOptimizer.sln --configuration Release --verbosity minimal
    run_cmd dotnet-test dotnet test src/SpatialLabsOptimizer.Tests/SpatialLabsOptimizer.Tests.csproj \
      --configuration Release --no-build --verbosity minimal
  else
    fail_gate "dotnet-sln" "SpatialLabsOptimizer.sln missing"
  fi
  if ! command -v npm >/dev/null 2>&1; then
    block_env "npm not found; required for catalog/worker gate"
  fi
  if [ -f site/catalog/package.json ]; then
    run_in_dir site/catalog catalog-test bash -c 'npm ci && npm test'
  fi
  if [ -f workers/steam-library/package.json ]; then
    run_in_dir workers/steam-library worker-test bash -c 'npm ci && npm test'
  fi
