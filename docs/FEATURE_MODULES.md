...
## 3D Game Optimizer (product stack)

| Layer | Desktop (WinUI) | Catalog (`site/catalog/`) | Worker |
|-------|-----------------|---------------------------|--------|
| View | `Views/*.xaml` | `src/*.ts` DOM modules | `src/http.ts` routes |
| Logic | `ViewModels/`, `Application/` | pure `src/` modules | `src/*.ts` handlers |
| Tests | `SpatialLabsOptimizer.Tests/` | Vitest `src/*.test.ts` | Vitest unit + worker pool |
| Wiring | `ServiceCollectionExtensions*.cs` | `catalog-bootstrap.ts` | `index.ts` export |

**Gate after each AGENT step:**
...
See `modules/winui/MODULE.md` and `docs/PARALLEL_AGENT_SCOPES.md`.
